...
**Flight-folder documents stored as data-URLs in the database:** Because the app has no backend and no Firebase Storage bucket, uploaded documents (passport scans, tickets, etc.) are kept inline as data-URLs in the Realtime Database. Images are downscaled to ≤1500px and re-encoded as JPEG (quality 0.82) on the client *before* upload to keep them small; PDFs are stored as-is (capped at 12MB). Each document carries an `owners` array of member IDs (or the special `all`), which is how ownership is tracked down to the individual child.

The heavy base64 payload of each file lives under its **own** key, `folder-docs/<id>`, while the `flight-folder` key holds only lightweight metadata (title, category, owners, file name, size, id). This split is deliberate: the entire folder is read-modify-written as one object on every change, so keeping the multi-MB blobs out of it stops a single large upload — e.g. an international driving licence — from pushing that one write past Firebase's size limit and failing (which previously left the document logged in the activity feed but never actually saved, then wiped from the local cache on the next sync). Blobs are fetched on demand and held in an in-memory cache for the session; they are **not** mirrored to `localStorage` (they'd exhaust its quota). Documents uploaded before this change keep their data-URL inline and are read transparently via `getDocData()`, so nothing has to be migrated. **Because `folder-docs` is a new top-level data key it must be present in the Firebase rules** (already added to `database.rules.json`) — without publishing that rule the locked root (`".write": false`) rejects blob writes and uploads fail with a clear "not saved" message instead of silently losing the file.
...
**Activity log & Undo:** A single `logActivity(action, entity, detail, who)` helper appends to the `activity-log` key (capped at the last 300 entries, written silently so it doesn't double-toast). It is called from every add/edit/delete site across the renderers, which is why mutations were instrumented individually rather than centralized in `saveKey()` — only the call site knows the human-readable description of what changed. The activity tab is organized like browser history (grouped by date, then by entity group, with search and per-group filters). Every **add** and **delete** is reversible: `saveKey()` snapshots the affected key's prior state, and `logActivity()` stores that snapshot under a new **`undo-store`** key (a compact, pruned store — newest 25 snapshots within ~4MB, skipping any single snapshot over ~2.5MB so large flight-folder images don't bloat the DB). Clicking ↩️ restores the key to its snapshot. **Because `undo-store` is a new top-level data key, it must be added to the Firebase rules** (already done in `database.rules.json`) — without it the locked root (`".write": false`) would reject undo-snapshot writes.

**Stale-while-revalidate reads:** Renderers read with `loadKey(key, fallback, {swr:true})`, which returns the `localStorage` cache instantly (so a page refresh paints with no network / App-Check round-trip) and revalidates in the background, re-rendering only when the server data actually changed and no input/modal is focused. Mutations still use the plain network-first `loadKey()` so the read-modify-write cycle starts from the freshest data.

**In-page document viewer:** Flight-folder documents open in an elegant full-screen lightbox (images + PDFs, with prev/next navigation, zoom and download) instead of a new browser tab. PDFs are rendered via a `blob:` URL (more reliable in an `<iframe>` than a large `data:` URL). The folder also has document search and a document-type filter.
